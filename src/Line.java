public class Line {
    ...
    {
        double ydif = end2.getY() - end1.getY();
        double xdif = end2.getX() - end1.getX();
        double dis  = Math.sqrt(Math.pow(xdif,2) + Math.pow(ydif,2));
        return dis;
    ...
    public Point midpoint()
    {
        double ydif = end2.getY() + end1.getY();
        double xdif = end2.getX() + end1.getX();
        Point mid = new Point(xdif/2, ydif/2);
        return mid;
    }

    public boolean sameLength(Line otherLine)
    {
        double len1 = this.distance();
        double len2 = otherLine.distance();
        return Math.abs(len1-len2) <= .001;
    }
    public double slope()
    {
        double m;
